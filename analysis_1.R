...
## trying to make it sexxy
fullset %>%
  boxplot(fam_income)

#this graph needs to be resized
ggplot(full, aes(x=full$`Population Estimate (BEA per Capita Denominator)`, y=full$`Estimated Median Family Income(HUD)`)) +
  geom_point(aes(size=full$`College Graduates Age 25 Up`)) +
